 ...
 *
 * 1) When a tribefire runtime property "TRIBEFIRE_EXTERNAL_PROPERTIES_LOCATION" is specified in catalina.properties, the file identified by this
 * property will be loaded and treated as a properties file (key/values pairs, line by line, separated by either "=" or ":")
 *
 ...
 *
 * 3) When TRIBEFIRE_EXTERNAL_PROPERTIES_LOCATION is specified, it may refer to a file, a URL where the properties can be downloaded or a classpath
 * resource.
 *
 *
 */
...
public class ExternalPropertiesConfigurator implements Configurator {
	...
	public void configure() throws ConfiguratorException {
		...
		String externalLocation = TribefireRuntime.getResolvedProperty(TribefireRuntime.ENVIRONMENT_EXTERNAL_PROPERTIES_LOCATION)
				.orElseGet(() -> null);
		...
		if (externalLocation == null || externalLocation.trim().length() == 0) {
			...
			logger.info(() -> "No " + TribefireRuntime.ENVIRONMENT_EXTERNAL_PROPERTIES_LOCATION
					+ " property defined. Trying to use the default location: '" + propertiesFile.getAbsolutePath() + "'");
		...
		if (externalFile != null && externalFile.exists()) {
			...
			try (InputStream is = new BufferedInputStream(new FileInputStream(externalFile))) {
				if (debug)
					logger.debug("Loading external properties from file: '" + externalFile.getAbsolutePath() + "'");
			...
			try (InputStream is = new URI(externalLocation).toURL().openStream()) {
			...
			if (!success) {
				logger.debug(
						() -> "The location " + externalLocation + " is neither a file path nor a URL. Trying to load it as a classpath resource.");
				try {
					ClassLoader classLoader = this.getClass().getClassLoader();
					try (InputStream resourceAsStream = classLoader.getResourceAsStream(externalLocation)) {
	...
	protected File resolveExternalPropertiesFile(final String externalLocation, Path confDir) {
		...
		if (f.isAbsolute()) {
			...
			logger.debug(
					() -> "The file path '" + f.getPath() + "' is relative. Using confDir as base: '" + confDir.toAbsolutePath().toString() + "'");
